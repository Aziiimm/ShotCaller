...
# team_points = player_df.groupby('team')['points_per_game'].sum().reset_index()
# team_points.columns = ['team', 'team_points_per_game']
# merged_df = pd.merge(merged_df, team_points, on='team', how='inner')

# merged_df['win_percentage'] = merged_df['wins'] / (merged_df['wins'] + merged_df['losses'])
# merged_df['match_outcome'] = merged_df['wins'].apply(lambda x: 1 if x > 0 else 0)
# merged_df['rebounds_per_game'] = merged_df['total_rebounds'] / (merged_df['wins'] + merged_df['losses'])
# merged_df['player_impact'] = (merged_df['points_per_game'] + merged_df['assists_per_game'] + merged_df['rebounds_per_game']) / merged_df['team_points_per_game']

# # print(merged_df[['player', 'points_per_game', 'assists_per_game', 'rebounds_per_game', 'player_impact']].head(50))
